...
type sqliteResourceServerScope struct {
	sqliteBaseModel
...
func mapResourceServerScope(resourceServerScope *repositories.ResourceServerScope) *sqliteResourceServerScope {
	return &sqliteResourceServerScope{
		sqliteBaseModel:  mapBase(resourceServerScope.BaseModel),
...
func (s *sqliteResourceServerScope) Map() *repositories.ResourceServerScope {
...
func (s *sqliteResourceServerScope) scan(row sqlitehelpers.Row, additionalPtrs ...any) error {
	ptrs := []any{
		&s.id,
		&s.auditCreatedAt,
		&s.auditUpdatedAt,
...
func (r *ResourceServerScopeRepository) List(ctx context.Context, filter *repositories.ResourceServerScopeFilter) ([]*repositories.ResourceServerScope, int, error) {
	...
	var totalCount int
	for rows.Next() {
		resourceServerScope := &sqliteResourceServerScope{}
...
func (r *ResourceServerScopeRepository) ExecuteInsert(ctx context.Context, tx *sql.Tx, resourceServerScope *repositories.ResourceServerScope) error {
	...
	resourceServerScope.SetVersion(version)
