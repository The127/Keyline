...
func mapUser(m *repositories.User) *sqliteUser {
	return &sqliteUser{
...
func (u *sqliteUser) Map() *repositories.User {
...
func (u *sqliteUser) scan(row sqlitehelpers.Row, filter *repositories.UserFilter, additionalPtrs ...any) error {
	ptrs := []any{
		&u.id,
		&u.auditCreatedAt,
		&u.auditUpdatedAt,
...
func (r *UserRepository) selectQuery(filter *repositories.UserFilter) *sqlbuilder.SelectBuilder {
	...
	if filter.HasSearch() {
		...
		s.Where(s.Or(
			s.Like("lower(username)", strings.ToLower(term)),
