...
func NewSessionService() middlewares2.SessionService {
...
func (s *sessionService) NewSession(ctx context.Context, virtualServerName string, userId uuid.UUID) (*utils.SplitToken, error) {
	...
	virtualServerFilter := repositories2.NewVirtualServerFilter().Name(virtualServerName)
...
func (s *sessionService) GetSession(ctx context.Context, virtualServerName string, id uuid.UUID) (*middlewares2.Session, error) {
	...
	case errors.Is(err, redis.Nil):
		...
		if dbSession != nil {
			...
			return middlewares2.NewSession(
				dbSession.UserId(),
...
func (s *sessionService) DeleteSession(ctx context.Context, virtualServerName string, id uuid.UUID) error {
	...
	virtualServerRepository := ioc.GetDependency[repositories2.VirtualServerRepository](scope)
	vsFilter := repositories2.NewVirtualServerFilter().Name(virtualServerName)
	...
	sessionRepository := ioc.GetDependency[repositories2.SessionRepository](scope)
	sessionFilter := repositories2.NewSessionFilter().Id(id)
...
func (s *sessionService) loadSessionFromDatabase(ctx context.Context, virtualServerName string, id uuid.UUID) (*middlewares2.Session, error) {
	scope := middlewares2.GetScope(ctx)

	virtualServerRepository := ioc.GetDependency[repositories2.VirtualServerRepository](scope)
	vsFilter := repositories2.NewVirtualServerFilter().
	...
	sessionRepository := ioc.GetDependency[repositories2.SessionRepository](scope)
	sessionFilter := repositories2.NewSessionFilter().
