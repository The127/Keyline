...
func mapProject(project *repositories.Project) *sqliteProject {
	return &sqliteProject{
		sqliteBaseModel: mapBase(project.BaseModel),
...
func (p *sqliteProject) Map() *repositories.Project {
...
func (p *sqliteProject) scan(row sqlitehelpers.Row, additionalPtrs ...any) error {
	ptrs := []any{
		&p.id,
		&p.auditCreatedAt,
		&p.auditUpdatedAt,
