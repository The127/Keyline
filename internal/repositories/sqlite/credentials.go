...
func mapCredential(credential *repositories.Credential) (*sqliteCredential, error) {
	...
	return &sqliteCredential{
		sqliteBaseModel: mapBase(credential.BaseModel),
...
func (c *sqliteCredential) Map() (*repositories.Credential, error) {
	var details any
...
func (c *sqliteCredential) scan(row sqlitehelpers.Row, additionalPtrs ...any) error {
	ptrs := []any{
		&c.id,
		&c.auditCreatedAt,
		&c.auditUpdatedAt,
