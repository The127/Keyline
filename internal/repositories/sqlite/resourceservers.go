...
type sqliteResourceServer struct {
	sqliteBaseModel
...
func mapResourceServer(resourceServer *repositories.ResourceServer) *sqliteResourceServer {
	return &sqliteResourceServer{
		sqliteBaseModel: mapBase(resourceServer.BaseModel),
...
func (r *sqliteResourceServer) Map() *repositories.ResourceServer {
...
func (r *sqliteResourceServer) scan(row sqlitehelpers.Row, additionalPtrs ...any) error {
	ptrs := []any{
		&r.id,
		&r.auditCreatedAt,
		&r.auditUpdatedAt,
...
func (r *ResourceServerRepository) List(ctx context.Context, filter *repositories.ResourceServerFilter) ([]*repositories.ResourceServer, int, error) {
	...
	var totalCount int
	for rows.Next() {
		resourceServer := &sqliteResourceServer{}
...
func (r *ResourceServerRepository) ExecuteInsert(ctx context.Context, tx *sql.Tx, resourceServer *repositories.ResourceServer) error {
	...
	resourceServer.SetVersion(version)
