...
type CreateApplicationRoleCommandSuite struct {
	suite.Suite
}

func TestCreateApplicationRoleCommandSuite(t *testing.T) {
	t.Parallel()
	suite.Run(t, new(CreateApplicationRoleCommandSuite))
}

func (s *CreateApplicationRoleCommandSuite) createContext(
...
func (s *CreateApplicationRoleCommandSuite) TestVirtualServerError() {
	// arrange
	...
	cmd := CreateApplicationRole{}

	// act
	_, err := HandleCreateApplicationRole(ctx, cmd)

	// assert
	s.Error(err)
}

func (s *CreateApplicationRoleCommandSuite) TestApplicationError() {
	// arrange
	ctrl := gomock.NewController(s.T())
	defer ctrl.Finish()
	...
	applicationRepository.EXPECT().Single(gomock.Any(), gomock.Any()).
		Return(nil, errors.New("error"))
...
func (s *CreateApplicationRoleCommandSuite) TestRoleError() {
	// arrange
	ctrl := gomock.NewController(s.T())
	defer ctrl.Finish()
...
func (s *CreateApplicationRoleCommandSuite) TestHappyPath() {
	// arrange
	ctrl := gomock.NewController(s.T())
	defer ctrl.Finish()
	...
	// act
	resp, err := HandleCreateApplicationRole(ctx, cmd)
