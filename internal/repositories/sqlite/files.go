...
func mapFile(file *repositories.File) *sqliteFile {
	return &sqliteFile{
		sqliteBaseModel: mapBase(file.BaseModel),
		name:            file.Name(),
...
func (f *sqliteFile) Map() *repositories.File {
...
func (f *sqliteFile) scan(row sqlitehelpers.Row, additionalPtrs ...any) error {
	ptrs := []any{
		&f.id,
		&f.auditCreatedAt,
		&f.auditUpdatedAt,
