...
func mapGroup(group *repositories.Group) *sqliteGroup {
	return &sqliteGroup{
		sqliteBaseModel: mapBase(group.BaseModel),
...
func (g *sqliteGroup) Map() *repositories.Group {
...
func (g *sqliteGroup) scan(row sqlitehelpers.Row, additionalPtrs ...any) error {
	ptrs := []any{
		&g.id,
		&g.auditCreatedAt,
		&g.auditUpdatedAt,
